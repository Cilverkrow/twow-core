# REF-018 disposable-MariaDB harness for the event-store migration, ported from
# twow-repo for provenance. Read this before running it.
#
# This is not a general runner. It is the evidence harness written for one
# specific task: moving the PlayerBot event store out of the character database
# into a dedicated `cv_bots` schema, and proving the migration was replay-safe,
# lossless and rejected the four seeded corruptions. It therefore requires
# inputs core does not carry:
#
#   * -MigrationFile: the cv_bots migration. Core has no such migration -- its
#     event store still defaults to `ai_playerbot_random_bots` in the character
#     database (PlayerbotAIConfig: AiPlayerbot.EventStoreTable).
#   * -DumpFile: a dated production dump, pinned by exact byte length and
#     SHA-256, from which the 206-line fixture is cut by line number.
#
# Without those two it cannot run, and neither can be reconstructed from this
# repository. It is kept here because it is the only written record of how the
# event-store adapter was verified end to end -- what was asserted, in what
# order, and what counted as a clean result -- and because
# playerbot_event_store_database_tests.cpp is still the binary it drives. If
# core ever repoints its event store, this is the script to adapt rather than
# the one to reinvent.
#
# Adjusted for core:
#   * C:\TW\disposable was hardcoded as the only permitted parent for the
#     disposable data directory; it is now -ApprovedDisposableParent, still
#     defaulting to that path.
#   * The adapter is now invoked with the event-store table as its second
#     argument. Core's contract reads the table from configuration rather than
#     baking it in, so the schema this harness migrates INTO has to be named
#     explicitly; passing `cv_bots`.`ai_playerbot_random_bots` reproduces
#     twow-repo's compiled-in behaviour exactly.
#   * STAGE=COMPLETE on the adapter's stderr is now required, matching the
#     guard core's CI puts on persistent_active_roster_database_tests. A crash
#     partway through the suite otherwise looks like a pass.
...
param(
    ...
    [string]$ApprovedDisposableParent = 'C:\TW\disposable',
    [string]$EventStoreTable = '`cv_bots`.`ai_playerbot_random_bots`',
    [int]$Port = 33319
)
...
function Invoke-Adapter([string]$Label) {
    ...
    $result = Invoke-CapturedNative -File $script:testExe -Arguments @($connection, $EventStoreTable) `
        -StdoutPath (Join-Path $script:evidence "$Label.stdout.log") `
        -StderrPath (Join-Path $script:evidence "$Label.stderr.log") `
    ...
    # The STAGE markers go to stderr; exit code 0 alone does not prove the last
    # scenario was reached.
    foreach ($stage in @('SCHEMA_PRECONDITION=OK', 'STAGE=COMPLETE')) {
        if ($result.Stderr -notmatch [regex]::Escape($stage)) {
            throw "$Label did not reach $stage."
        }
    }
    foreach ($required in @(
...
$approvedDisposableParent = [IO.Path]::GetFullPath($ApprovedDisposableParent)
