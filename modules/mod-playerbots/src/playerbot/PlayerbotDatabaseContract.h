...
{
// The event-store table name is a deployment choice, not a compile-time
// constant: some deployments keep it inline in the character database
// (the historical default, `ai_playerbot_random_bots`), others repoint it
// at a dedicated schema. Read it fresh from config on every call so a
// rehash (.reload config) picks up a change without a restart.
inline std::string const& EventStoreTable()
{
    return sPlayerbotAIConfig.eventStoreTable;
}
