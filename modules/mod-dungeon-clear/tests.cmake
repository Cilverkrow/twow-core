...
# NOT from mod-dungeon-clear.cmake, mirroring modules/mod-playerbots/tests.cmake
# next door. The module .cmake is only read when the module's linkage is not
# "disabled" (modules/CMakeLists.txt), so tests defined there could not be built
# in the default configuration — the one CI uses. It is also included twice per
# configure (the DISCOVERY and POST_TARGETS phases), which would have defined
# every target twice; the declaration that used to live there worked around that
# with cmake_language(DEFER ...) rather than by moving out.
#
# That old, single `dungeon_clear_tests` target could never have been built at
# all: it linked game-interface and compiled src/test/mocks/TestMap.cpp, and
# neither exists in this tree — they are AzerothCore's test scaffolding. ctest
# consequently discovered zero tests.
#
# ---------------------------------------------------------------------------
...
# The header search path the shipping module compiles against.
#
# modules/CMakeLists.txt builds exactly this list as MODULES_COMMON_INCLUDES,
# but that is a plain set() in the modules/ subdirectory scope and this file is
# included from the root — where the variable does not exist. Spelled out here
# instead, rooted at TW_CORE_ROOT rather than CMAKE_SOURCE_DIR so it still points
# at the core when a platform repository consumes it with add_subdirectory().
#
# The tests must resolve the same headers as the code under test; the core
...
# (produced by tools/slice_mapdata.py). Client-derived map data is never
# committed, so it is normally absent and those cases GTEST_SKIP.
